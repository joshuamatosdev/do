---
name: bb-methodology
description: Master workflow for bug bounty hunting sessions. Covers the 5-phase non-linear workflow, the daily mindset discipline (Define/Select/Execute), 4 thinking domains, the amateur-vs-pro comparison, and tool routing by phase. Use at the START of any hunting session, when switching targets, or when you feel lost about what to do next. Also use when asking "what should I do next" or "where am I in the process." Trigger phrases include "/bb-methodology".
---
...
a written bug-bounty scope, or a sanctioned engagement. Before any active step, confirm
authorization. See `do:security-recon` for the authorization gate and active-tool rules.
...
This skill is the master workflow for hunting sessions. Use it with the sibling skill
`do:security-arsenal` (payloads and bypass tables) and the `do:red-blue` orchestrator (static or
local-dynamic mode); `do:security-recon` is the authorization gate. Triage and validate a finding —
impact, reproducibility, scope — before you report it.
...
4. **Identity**: Anonymous or authenticated? If the bugs you're hunting need a
   ...
   load auth **once** at session start — keep credentials in your engagement log
   (never hard-coded in tool arguments). Every downstream tool (httpx, katana,
   ffuf, nuclei, dalfox, PoC verifiers) should send those headers automatically
   so audit log entries are tied to a stable session reference.
...
**Pre-report gate:**
```
Run the 7-Question triage gate
+-- All 7 pass? -> Write report
+-- Any fail? -> KILL the finding. Don't waste time.
+-- Borderline? -> treat as fail unless you can name the missing evidence and get it; lean KILL
```

**Report** (use the `do:security-recon` Phase 5 report shape):
```
Report shape:
