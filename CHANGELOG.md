...
- **The three Codex Stop hooks are merged into one** — `codex-adversarial-review.sh` + `codex-integrity-review.sh` + `codex-later-stop.sh` become a single plugin-declared `codex-stop.sh`. Behavior-preserving: each concern self-gates on its own module (codex-integrity / codex-later stay independently installable) and keeps its own behavior and Codex path; when both block in one turn the reasons combine into a single block. Result: one Stop registration and one transcript classification instead of three. `codex-later` no longer injects a hook into `settings.json` (it is plugin-declared via the merged hook); existing installs are migrated on update.
